...
export default function DateRangeInput({ onDateChange }: Props) {
  ...
  const [date1, setDate1] = useState<string>(format(startDate, 'dd-MM-yyyy'))
  const [date2, setDate2] = useState<string>(format(endDate, 'dd-MM-yyyy'))
  const [error, setError] = useState<string | null>(null)
  const date2Ref = useRef<HTMLInputElement>(null)
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 items-center">
        <DateRangeSelect onChange={setInvoiceRange} initialValue="today" className='w-40 text-xl border-none shadow-none focus:outline-none focus-visible:outline-none focus-within:outline-none'   /> :
        <FloatingInput
          label="Start Date"
          type="text"
          placeholder="DD-MM-YYYY"
